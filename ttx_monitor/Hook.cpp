...
/* by yksoft1 */
static LPGetDriveTypeA __GetDriveTypeA = NULL;
...
//	__GetKeyLicense = (LPGetKeyLicense) HookIt("IpgExKey.dll", "_GetKeyLicense@0", Hook_GetKeyLicense);
	...
	__XHOOKn("kernel32.dll", GetDriveTypeA);
	...
	// As duas versões são retrocompatú“eis.
...
BOOL __stdcall Hook_GetCommModemStatus(HANDLE hFile, LPDWORD lpModemStat)
{
	...
	// SEdevemos retornar esse valor quando a JVS receber o comando de Address
	if (is_addressed())
		*lpModemStat = 0x10;
	else
		*lpModemStat = 0;
...
BOOL __stdcall Hook_EscapeCommFunction(HANDLE hFile, DWORD dwFunc)
{
 ...
 UINT __stdcall Hook_GetDriveTypeA(LPCSTR root)
{
	 return DRIVE_REMOVABLE; /*make yksoft1's hacked cvgame.exe load faster*/
}
/*
0xfd
...
unsigned short ds2GetPlayerInput(int player)
{
...
unsigned char ds2GetCoin()
{
	unsigned char temp = 0;
	if (inputMgr.GetState(P2_COIN)||inputMgr.GetState(P1_COIN))
		temp=0xFF;
	return temp;
}
...
void ds2Reply()
{
	static unsigned short p1o,p2o;
	static unsigned char to,co;
	static int coininit;
	unsigned short p1,p2;
	unsigned char t,c;
	
	p1=ds2GetPlayerInput(1);
	p2=ds2GetPlayerInput(2);
	t=ds2GetTestBtn();
	c=ds2GetCoin();
	if(p1!=p1o)
	{
		replyBuffer.push('Q');
		replyBuffer.push((LOBYTE(p1)));
		replyBuffer.push((HIBYTE(p1)));

		p1o=p1;

/*		logmsg("P1 status changed to %x\n",p1);*/
	}
	if(p2!=p2o)
	{
		replyBuffer.push('R');
		replyBuffer.push((LOBYTE(p2)));
		replyBuffer.push((HIBYTE(p2)));

		p2o=p2;
		/*logmsg("P2 status changed to %x\n",p2);*/
	}
	if(t!=to)
	{
		replyBuffer.push('P');
		replyBuffer.push(t);
		to=t;
/*		logmsg("TEST btn status changed to %d\n",t==1);*/
	}
	if(c!=co) 
	{
		if(coininit)
