...
: is_loaded(FALSE)
{
	...
	_tprintf(_T("Creating process %s...\n"), name);
	...
	is_loaded = CreateProcess(name,
		...
		&pi);
	if (!is_loaded)
		Error(_TEXT("Process not loaded!"));
...
// complicadas na internet, acabei por "pegar" o conceito, que na verdade ÅE// muito simples. O que precisamos fazer ÅEcarregar o processo em modo suspenso,
// ent„o, usamos VirtualAllocEx pra alocar memÛria virtual para um string, que 
// nesse caso ÅEo nome da DLL a ser carregada, lembrando que ÅEpreciso utilizar
// a flag PAGE_EXECUTE_READWRITE. Depois de alocarmos a memÛria, escrevemos a
// string utilizando WriteProcessMemory.
//    Agora criaremos uma thread remota que executarÅEa funÁ„o LoadLibrary no
// endereÁamento do processo com o parametro apontando para o endereÁo da string
...
int _tmain(int argc, _TCHAR* argv[])
{
	LProcess *proc = NULL;
	if (argc < 2)
		proc = new LProcess(_TEXT("cvgame.exe"));
	else
		proc = new LProcess(argv[1]);
	proc->InjectDLL(_T("ds2_monitor.dll"));
	proc->Resume();
	proc->Wait();
	return 0;
}
