...
int TTX_InputManager::Poll()
{
	...
	for (int i = P1_START; i < __INPUT_MAX__; i++) {
		...
		// verifica se Eum comando de origem do joystick
...
int TTX_InputManager::Load()
{
	...
	if (!fp) {
		logmsg("Não foi possú“el ler o arquivo: %s!\n", MONITOR_INPUT_FILENAME);
		return 1;
	}
...
int TTX_InputManager::Save()
{
	...
	if (!fp) {
		logmsg("Não foi possú“el criar o arquivo: %s\n", MONITOR_INPUT_FILENAME);
		return 1;
	}
...
void TTX_InputManager::Setup()
{
	...
	if (!clsId) {
		...
		if (err != ERROR_CLASS_ALREADY_EXISTS) {
			logmsg("Não foi possú“el registrar a classe! %d\n", err);
			ExitProcess(-1);
		}
	}
	hSelfWin = CreateWindowEx(0,
		...
		NULL);
	if (!hSelfWin) {
		logmsg("Não foi possú“el criar a janela!\n");
		ExitProcess(-2);
	}
	DImagicCall = 1;
